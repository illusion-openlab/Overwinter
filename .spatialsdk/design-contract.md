...
| 花朵无敌态 | **移到 V2** | 本版花朵只加分（+5）。3 秒无敌及其整套表现（暖白光晕 / 余晖 / 雪雾 / HUD 倒计时环 / 霜华退去）连同截图验证一并延后。视觉稿里的「状态 03」保留作 V2 参考 |
...
3. **v1 有「撞击」音，现在没有。** 用户给的四个用途里没有撞击，坠落对应的是结算态。**撞枝 −25° 目前没有任何听觉反馈**——这是本作最重要的负反馈，属已知缺口，待用户决定是否补素材。
4. `飞翔.mp3` 原文件是**三次连续振翅共 2.4s**。每次点击播完整段会听到三下、连点还会糊，所以只取第 2 次振翅（0.855s~1.215s）并加两端淡入淡出，落成 `flap.wav`。原文件保留在用户本地，未入库。
